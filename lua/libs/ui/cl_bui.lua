...
function BUi:FormatMoney(str)
    local k = #tostring(str)
    local k2 = k - 1
    local full = k2 / 3 - (k2 / 3) % 1
    local cache = k - full * 3

    return "$" .. string.sub(str, 1, cache) .. (str > 1000 and ("" .. string.sub(str, cache + 1, cache )) or "") .. (ntl[k] or "")
end

function BUi:Scale( i )
    return math.max( i * ( ScrH() / 1080 ), 1 )
end

function BUi:RegisterFont( name, font, size, weight )
    if !name then print("[BUI:RegisterFont]", "missing argument #1 (string name)" ) return end
    if !font then print("[BUI:RegisterFont]", "missing argument #2 (string font)" ) return end
    if !size then print( "[BUI:RegisterFont]", "missing argument #3 (integer size)" ) return end
...
function BUi:CreateFont( name, font, size, weight )
    scaledFonts[ name ] = {
        font = font,
        size = size,
        weight = weight,
...
hook.Add( "OnScreenSizeChanged", "VectivusLib:RegisterFont", function()
    timer.Simple( 1, function()
        for k, v in pairs( scaledFonts ) do
            print( "[ BUI:Fonts - Updated]", k )
            BUi:CreateFont( k, v.font, v.size, v.weight )
        end
    end )
end )
...
BUi.DrawCircle = function(x, y, r, col)
    local circle = {}
...
BUi.DrawArc = function(x, y, ang, p, rad, color, seg)
	seg = seg || 80
    ang = (-ang) + 180
    local circle = {}
...
BUi.LerpColor = function(frac, from, to)
	return Color(
...
BUi.Doclick = function(s) return input.IsMouseDown(MOUSE_LEFT) and s:IsHovered() end
...
classes.Shadow = function(pnl, op,ins,sp,blur,offset)
	pnl:On("Paint", function(s, w, h)
		local x, y = pnl:LocalToScreen(0, 0)
		if BUi.shadows.Start(pnl) then
			BUi.shadows.SetOpacity(op or 100)
			BUi.shadows.SetIntensity(ins or 3)
			BUi.shadows.SetSpread(sp or 3)
			BUi.shadows.SetBlur(blur or 1)
			BUi.shadows.SetOffset(blur or 1, blur or 1) 
	
			BUi.shadows.SetRelative(pnl)
			BUi.shadows.SetRenderedAt(x, y)
			draw.RoundedBox(8, x, y, w, h, Color(22, 22, 22))
...
classes.BarHover = function(pnl, col, height, speed,r)
	col = col || Color(255, 255, 255, 255)
	height = height || 2
	speed = speed || 6

	pnl:SetupTransition("BarHover", speed, BUi.HoverFunc)
	pnl:On("Paint", function(s, w, h)
		local bar = math.Round(w*s.BarHover)
		
		draw.RoundedBox(r or 0,w/2-bar/2, h-height, bar, height,col)
	end)
end
...
classes.FillHover = function(pnl, col, dir, speed, mat)
	...
	pnl:SetupTransition("FillHover", speed, BUi.HoverFunc)
	pnl:On("PaintOver", function(s, w, h)
		surface.SetDrawColor(col)
...
classes.SideBlock = function(pnl, col, size, side,r)
	col = col || Color(255, 255, 255, 255)
	size = size || 3
	side = side || LEFT
	...
	pnl:On("Paint", function(s, w, h)
		...
		if(side == LEFT) then
			draw.RoundedBox(r or 0,0, 0, size, h,col)
		elseif(side == TOP) then
			draw.RoundedBox(r or 0,0, 0, w, size,col)
		elseif(side == RIGHT) then
			draw.RoundedBox(r or 0,w-size, 0, size, h,col)
		elseif(side == BOTTOM) then
			draw.RoundedBox(r or 0,0, h-size, w, size,col)
		end
	end)
end
...
classes.Blur = function(pnl, amount,roundness)
	pnl:On("Paint", function(s, w, h)
		...
		surface.SetMaterial(BUi.Grad["Blur"])

		for i = 1, 3 do
			BUi.Grad["Blur"]:SetFloat("$blur", (i / 3) * (amount or 8))
			BUi.Grad["Blur"]:Recompute()
			...
			BUi.masks.Source()
			draw.RoundedBox(roundness or 0, 0, 0, w, h, color_white)
			BUi.masks.End()
		end
	end)
end
...
classes.CircleHover = function(pnl, col, speed, trad)
	...
	pnl:SetupTransition("CircleHover", speed,BUi.HoverFunc)
	pnl:On("Think", function(s)
		if(s:IsHovered()) then
...
classes.CircleFadeHover = function(pnl, col, speed)
	...
	pnl:SetupTransition("CircleFadeHover", speed, BUi.HoverFunc)
	pnl:On("Paint", function(s, w, h)
		draw.NoTexture()
...
classes.CircleExpandHover = function(pnl, col, speed)
	...
	pnl:SetupTransition("CircleExpandHover", speed, BUi.HoverFunc)
	pnl:On("Paint", function(s, w, h)
...
classes.Stick = function(pnl, dock, margin,l,t,r,b, dontInvalidate)
	dock = dock || FILL
	margin = margin || 0

	pnl:Dock(dock)
	pnl:DockMargin(l or margin,t or margin,r or margin,b or margin)
...
function meta:BUi()
	self.Class = function(pnl, name, ...)
		...
		assert(class,"[BUI]" .. "Class "..name.." does not exist.")
